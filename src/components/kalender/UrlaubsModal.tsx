...
import { urlaubAnlegen } from '@/app/(app)/kalender/actions';

interface UrlaubsModalProps {
  vonDatum?: string;   // YYYY-MM-DD vorausgefüllt
  onSchließen: () => void;
...
export default function UrlaubsModal({
  vonDatum,
  onSchließen,
  feiertage,
}: UrlaubsModalProps) {
  ...
  const [bis, setBis] = useState(vonDatum ?? '');
  const [typ, setTyp] = useState<'urlaub' | 'sonderurlaub'>('urlaub');
  ...
  // Halber Tag toggling: Bis-Datum auf Von-Datum setzen
  ...
  const handleAbsenden = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const ergebnis = await urlaubAnlegen(formData);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-700">
        <h2 id="modal-titel" className="text-base font-semibold text-gray-900 dark:text-slate-100">
          Urlaub eintragen
        </h2>
        <button
          type="button"
      ...
      <form onSubmit={handleAbsenden} className="px-6 py-5 space-y-4">
        ...
        <div className="flex gap-3 justify-end pt-1">
          ...
          >
            {lädt ? 'Wird gespeichert...' : 'Eintragen'}
          </button>
        </div>
      </form>
    </dialog>
